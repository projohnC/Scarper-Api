...
import { Plus, ThumbsUp, ChevronDown, MonitorPlay } from "lucide-react";
      ...
      <div className="absolute inset-0 z-10 hidden scale-0 bg-zinc-900 opacity-0 transition duration-300 group-hover:scale-110 group-hover:opacity-100 sm:block rounded-md shadow-xl pointer-events-none group-hover:pointer-events-auto">
        ...
        <div className="p-2 lg:p-4">
          <div className="flex items-center gap-3">
            <button
              onClick={handlePlay}
              className="flex h-6 w-6 items-center justify-center rounded-full bg-white transition hover:bg-neutral-300 lg:h-10 lg:w-10"
              title="Play in VLC"
            >
              <MonitorPlay className="h-4 w-4 text-black lg:h-6 lg:w-6" />
            </button>
